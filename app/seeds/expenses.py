...
def seed_expenses():
    ...
    expenses3 = Expense(description="Internet", amount=120, date=faker.date(
        pattern='-%m-%d%Y', end_datetime=None), notes="Comcast", group_id=3,
        user_id=5)
    expenses4 = Expense(description="Groceries", amount=200, date=faker.date(
        pattern='-%m-%d%Y', end_datetime=None), notes="Trip to Trader Joes",
        group_id=4, user_id=1)
    expenses5 = Expense(description="Uber", amount=30, date=faker.date(
        pattern='-%m-%d%Y', end_datetime=None), notes="Uber to bar",
